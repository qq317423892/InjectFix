...
namespace IFix
{
    ...
    class CodeTranslator
    {
            ...
            GenerateConfigure configure, ProcessMode mode)
        {
            ...
            //{
            //    if(!(isCompilerGenerated(type) || isNewClass(type)) && !type.HasGenericParameters)
            //    {
            //        foreach(var method in type.Methods)
            //        {
            //            if(!method.IsConstructor && !isCompilerGenerated(method) && !method.HasGenericParameters)
            //            {
